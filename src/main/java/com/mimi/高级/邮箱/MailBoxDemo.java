...
class CustormMailBox {
    static class BusinessMsgQueue implements MessageQueue {
        ...
        // 假如在投递消息的过程中 Actor 不可用了，
        // 那么没有投递成功的消息会通过 cleanUp 进入死信队列
        @Override
...
//        ActorRef simpleActor = sys.actorOf(Props.create(SimpleActor.class).withMailbox("test-mainbox"), "simpleActor");
