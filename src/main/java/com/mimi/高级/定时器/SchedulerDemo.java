...
public class SchedulerDemo {
    ...
    public static void main(String[] args) {
        ...
        system.scheduler().scheduleOnce(Duration.create(3, "s"),
                target, "hello", system.dispatcher(), sender);


        // 延迟 1 秒之后给 refTarget 发送一条消息（“hello”），然后每隔 3 秒执行一次（发消息）。
        // 在不需要该定时任务时，可以调用 cancel 方法将其取消。
        Cancellable cancelHandler = system.scheduler().schedule(Duration.create(1, "s"),
                Duration.create(3, "s"), target, "hello", system.dispatcher(), sender);
